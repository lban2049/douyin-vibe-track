...
从 `~/douyin-vibe-track/config.json` 读取 `reports_dir`。如果缺省或为空字符串，报告默认写入工作目录下的 `reports/`。用户希望把报告和工作目录分开时，Agent 直接修改 `config.json` 中的 `reports_dir`：

```json
{
  "reports_dir": "/absolute/path/to/douyin-reports"
}
```

`reports_dir` 支持 `~`。如果填写相对路径，会按工作目录解析，例如 `"reports_dir": "../douyin-reports"` 会写入 `~/douyin-reports/`。
